...
import pandas as pd
import os
root = 'C:/Users/Sushant/EMOJINATOR/gestures' # or ‘./test’ depending on for which the CSV is being created
...
# so taking the 9th value of the folder gave the digit (i.e. "./train/8" ==> 9th value is 8), which was inserted into the first column of the dataset.
        value = np.hstack((directory[10:],value))
        df = pd.DataFrame(value).T
        df = df.sample(frac=1) # shuffle the dataset
        with open('train_foo1.csv', 'a') as dataset:
